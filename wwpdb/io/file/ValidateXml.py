...
import os, operator, sys, string, traceback
        ...
        for node in self.__doc.getElementsByTagName('ModelledSubgroup'):
            ...
            for childnode in node.childNodes:
                if childnode.nodeType != node.ELEMENT_NODE:
                    continue
                #
                if not childnode.tagName in self.__outlierMap:
                    continue
                #
                if not residueInfo:
                    residueInfo = self.__getMapInfo(node, items)
                ...
                #        continue
                    #
                # skip bonds between standard residues in reporting, these are better captured in pdbx_validate_polymer_linkage - the validation is limited to a maxiumum distance of 1.999 Angstroms 
                ...
                if childnode.tagName == 'clash':
                    atom_dict = {'dist': dir['dist'], 'atom': dir['atom'],
        ...
        if self.clashMap:
            for o in self.clashMap:
                ...
                if len(self.clashMap[o]) > 1:
                    clashitem = {'res1model': self.clashMap[o][0]['model'],
